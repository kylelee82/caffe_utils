...
model_weights_prefix = './models/statefarm/snapshot_fold'
batch_size = 16
num_folds  = 8

for fold_cnt in range(num_folds):

	print('Predicting test images for fold {}...'.format(fold_cnt))

	net = caffe.Net(model_def, model_weights_prefix+str(fold_cnt)+".best_val.caffemodel", caffe.TEST)
	...
	if not os.path.isdir(subm_path):
		os.mkdir(subm_path)
	sub_file = os.path.join(subm_path,'fold' + str(fold_cnt) + '_' + str(now.strftime("%Y-%m-%d-%H-%M")) + '.csv')
